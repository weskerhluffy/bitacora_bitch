...
function clearErrorLabelsXHTML(form) {
	...
	for (i = 0; i < elements.length; i++) {
		...
		if (parentEl && parentEl.cells) {
			var labelRow = parentEl.cells.length > 1 ? parentEl : StrutsUtils
					.previousElement(parentEl, "tr");
			if (labelRow) {
...
function addErrorXHTML(e, errorText) {
	try {
		...
		span.setAttribute("className", "errorMessage"); // ie hack cause ie does
														// not support
														// setAttribute
		...
		// if labelposition is 'top' the label is on the row above
		var labelRow = row.cells.length > 1 ? row : StrutsUtils
				.previousElement(tr, "tr");
